...
GitOps в **devops-time-host** → `apps/lipetsk-gas-monitor/`: 1 pod (web+collector), PVC, Ingress gas.qantrix.ru, Flux image automation.  
...
`SOURCE_URL`, `DATA_DIR`, `COLLECT_INTERVAL_SECONDS=300`, `SAVE_SNAPSHOT_FILES` (true dev / false prod), `BOOTSTRAP_ENABLED`, `PORT=18743`.
